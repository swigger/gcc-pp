  ...
  if (pfile->comments.allocated == 0)
    {
      ...
      pfile->comments.entries = (cpp_comment *) malloc
  ...
  if (pfile->comments.count == pfile->comments.allocated)
    {
      ...
      pfile->comments.entries = (cpp_comment *) realloc
...
cpp_output_line_to_string (cpp_reader *pfile, const unsigned char *dir_name)
{
  ...
  unsigned char *result = (unsigned char *) malloc (alloced);
  ...
  while (token->type != CPP_EOF)
      ...
      if (out + len > alloced)
	{
	  ...
	  if (out + len > alloced)
	    alloced = out + len;
	  result = (unsigned char *) realloc (result, alloced);
	}
